...
if('final_data.RData' %in% dir()){
  ...
  school_locations$SCHOOL_CONDITIONS <-
    ifelse(school_locations$SCHOOL_CONDITIONS == 1, 'Caniço e palha',
           ifelse(school_locations$SCHOOL_CONDITIONS == 2, 'Caniço e chapa de zinco',
                  ifelse(school_locations$SCHOOL_CONDITIONS == 3, 'Matope e palha',
                         ifelse(school_locations$SCHOOL_CONDITIONS == 4, 'Matope e chapa de zinco',
                                ifelse(school_locations$SCHOOL_CONDITIONS == 5, 'Madeira e zinco',
                                       ifelse(school_locations$SCHOOL_CONDITIONS == 6, 'Convencional e outras precárias',
                                              ifelse(school_locations$SCHOOL_CONDITIONS == 7, 'Todas de material convencional',
                                                     ifelse(school_locations$SCHOOL_CONDITIONS == 8, 'Outro', NA))))))))
  
  # Clean up type of latrine
  school_locations$TYPE_OF_LATRINE <-
    ifelse(school_locations$TYPE_OF_LATRINE == 1, 'Retrete ligada a fossa séptica',
           ifelse(school_locations$TYPE_OF_LATRINE == 2, 'Latrine Melhorada',
                  ifelse(school_locations$TYPE_OF_LATRINE == 3, 'Latrina tradicional melhorada',
                         ifelse(school_locations$TYPE_OF_LATRINE == 4, 'Latrina tradicional não melhorada',
                                ifelse(school_locations$TYPE_OF_LATRINE == 5, 'Sem latrina', NA)))))
  
  # Clean up lunch #
  school_locations$LUNCH <- ifelse(school_locations$LUNCH == 1, TRUE,
                     ifelse(school_locations$LUNCH == 2, FALSE,
                            NA))
  
  # Remove duplicates
  school_locations <- school_locations[!duplicated(school_locations$SCHOOL_NAME),]
  ...
  ## REMOVE ALL WEEKS FOR WHICH THE ABSENTEEISM RATE < 1
  turmas_2015 <- df %>%
    group_by(DISTRICT, 
             SCHOOL_NAME,
             GRADE,
  ...
  rm(school_locations)
  schools <- schools[!duplicated(schools$SCHOOL_NAME),]
  ...
  # 4. census
  # 5. schools
  rm(ll, locations_magude, j)
  
  # Remove one duplicate
  schools <-
    schools %>%
    mutate(remove = SCHOOL_NAME == 'EPC Mulembja ') %>%
    filter(!remove) %>%
    dplyr::select(-remove)
  
  # Get a map of magude and manhica
